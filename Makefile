...
DO_PDFLATEX_RAW = $(TEX_INTERPRETER) -shell-escape -interaction=nonstopmode purcell >err
...
BOOK = purcell
GENERIC_OPTIONS_FOR_CALIBRE =  --authors "Edward Purcell" --language en --title "Electricity and Magnetism" --toc-filter="[0-9]\.[0-9]" --title="Electricity and Magnetism"
...
default:
	...
	makeindex purcell.idx >/dev/null
...
clean:
	# Cleaning...
	@rm -f purcell.pdf purcell_lulu.pdf
	@rm -f temp.tex
	@rm -f ch*/ch*temp.tex 
...
post:
	cp purcell.pdf ~/Lightandmatter/purcell

prepress:
	make preflight_figs
	pdftk purcell.pdf cat 3-end output purcell_lulu.pdf
